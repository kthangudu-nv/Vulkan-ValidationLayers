...
TEST_F(VkSyncValTest, SyncLayoutTransition) {
    ...
    // vkCmdPipelineBarrier: Hazard WRITE_AFTER_WRITE for image barrier 0 VkImage 0xa540ac0000000009[].
    // Access info (usage: SYNC_IMAGE_LAYOUT_TRANSITION, prior_usage: SYNC_IMAGE_LAYOUT_TRANSITION,
    // write_barriers: Unhandled SyncStageAccess, command: vkCmdEndRenderPass, seq_no: 6, reset_no: 1).
    m_errorMonitor->ExpectSuccess();
    const VkImageMemoryBarrier preClearBarrier2 = {VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
                                                  ...
                                                  image_input.handle(),
                                                  full_subresource_range};
    vk::CmdPipelineBarrier(m_commandBuffer->handle(), VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0,
                           0, 0, 0, 1, &preClearBarrier2);
    m_errorMonitor->VerifyNotFound();
...
TEST_F(VkSyncValTest, SyncMassiveSubpassDependencies) {
    ...
    // Massive loops in AccessContext::ResolvePreviousAccess & AccessContext::ResolveAccessRange
    // The test has 21 subpasses.It runs more and more loops in every next subpass.

    // This test is not the totally the same with CTS, but similar. Massive dependencies cause massive loops.
    // In CTS, the issue happens in CmdNextSubpass. In this test, it happens in EndRenderPass.
